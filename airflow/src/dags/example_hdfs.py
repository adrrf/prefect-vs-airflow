...
default_args = {"owner": "airflow", "start_date": datetime(2024, 3, 1), "retries": 1}
# Inicializar el DAG
dag = DAG(
    "dag_hdfs_pipeline",
...
tarea_descargar = PythonOperator(
    task_id="descargar_datos", python_callable=descargar_datos, dag=dag
)
tarea_subir_hdfs = PythonOperator(
    task_id="subir_a_hdfs", python_callable=subir_a_hdfs, dag=dag
)
tarea_contar_lineas = PythonOperator(
    task_id="contar_lineas_hdfs", python_callable=contar_lineas_hdfs, dag=dag
)
